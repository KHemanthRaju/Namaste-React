...
- What is '.gitignore'? What we should add and do not add into it?
    ...
    - For example in gitignore we add node-modules because node-modules can be autogenrated at production server. We should not add packagejson file because those are needed at server to get the packages.
